...
def generateJoined(
    android_dict, cupertino_dict, androidExtend, cupertinoExtend, common
):
    ...
    res.append(
        "class Adaptive"
        + name
        + " extends AdaptiveBase<%s, %s> {" % (androidExtend, cupertinoExtend)
    )
...
def generateCreateWidgetFunction(platform, extendName, platform_dict, common):
    ...
    res = []
    res.append("%s create%sWidget(BuildContext context) {" % (extendName, platform))
...
for key in android_processed.keys():
    if (
        key in cupertino_processed
        and android_processed[key].getType() == cupertino_processed[key].getType()
    ):
        common.append(key)
...
res.append(
    generateJoined(
        android_processed, cupertino_processed, androidExtend, cupertinoExtend, common
    )
)
res.append("")
# Generate android create widget function
res.append(
    generateCreateWidgetFunction("Android", androidExtend, android_processed, common)
)
res.append("")
# Generate iOS create widget function
res.append(
    generateCreateWidgetFunction(
